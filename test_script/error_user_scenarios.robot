...
*** Test Cases ***
[Expected to FAIL]Verify user should able to remove selected products from inventory page
    [Tags]    eu-1    error_user     regression     negative_scenario
...
[Expected to FAIL]Verify that user should able to fill user information correctly
    [Tags]    eu-2    error_user     regression     negative_scenario
...
[Expected to FAIL]Verify that user should not place order for invalid order process
    [Documentation]    The user information input box validation is tested in another scenario
    [Tags]    eu-3    error_user     regression     negative_scenario
